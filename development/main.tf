...
resource "aws_s3_object" "webflow_site_files" {
  ...
  source = data.archive_file.wf_site.output_path
  etag   = filemd5(data.archive_file.wf_site.output_path)
...
resource "aws_elastic_beanstalk_application_version" "app_version" {
  ...
  bucket      = aws_s3_bucket.webflow_bucket.bucket
...
resource "aws_elastic_beanstalk_environment" "app_environment" {
  name                = "wf-site-dev"
  application         = aws_elastic_beanstalk_application.app.name
  solution_stack_name = "64bit Amazon Linux 2023 v4.13.7 running Python 3.13"
